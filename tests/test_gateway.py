...
from bridger.gateway import GatewayAlreadyExists, GatewayData, GatewayManagerEMQX
...
def test_create_gateway_user(gateway_manager, emqx_mock):
    ...
    emqx_mock.create_user_authorization_rules_built_in_database.assert_called_once_with(
        gateway.user_string, [{"action": "all", "topic": "egr/home/2/e/LongFast/!1a2b3c4d", "permission": "allow"}]
    )


# Test create_gateway_user when user already exists
def test_create_gateway_user_already_exists(gateway_manager, emqx_mock):
    emqx_mock.create_user.side_effect = HTTPError("User already exists")

    with pytest.raises(GatewayAlreadyExists):
