...
from bridger.gateway import GatewayAlreadyExists, GatewayManagerEMQX, emqx
...
def check_gateway_owner(interaction: Interaction) -> bool:
    ...
    owner = interaction.client.get_user(gateway.owner.discord_id)
...
class MQTTCog(commands.GroupCog, name="bridger-mqtt"):
    ...
    async def cog_app_command_error(self, interaction: Interaction, error: app_commands.AppCommandError):
        ...
        if isinstance(error, app_commands.errors.CommandInvokeError):
            if isinstance(error.original, GatewayAlreadyExists):
            ...
            await interaction.response.send_message(
                f"Gateway not found: {error}", ephemeral=True, delete_after=self.delete_after
            )
